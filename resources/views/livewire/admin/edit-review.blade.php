<div class="flex flex-col items-center px-4 md:px-6 py-4">
    ...
    <div class="w-full">
        ...
        <div class="grid grid-cols-2 items-center justify-center gap-2 mt-4">
            <div class="col-span-2">
                <!-- Preview -->
                @if ($photo)
                    <img src="{{ $photo->temporaryUrl() }}" alt="Selected photo" class="w-20 h-20 rounded-full mb-4">
                @else 
                <img src="{{$review->photoUrl}}" alt="review photo"  class="w-20 h-20 rounded-full mb-4">
                @endif
    ...
    <div class="w-full flex justify-end items-center mt-3 pt-3 border-t border-gray-200">
        <x-secondary-button class="rounded-xl" wire:click="save">update review</x-secondary-button>
    </div>
</div>
