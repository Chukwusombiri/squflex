                                ...
                                <div class="grid grid-cols-4 gap-6">
                                    @if (count($plans) > 0)
                                        @foreach ($plans as $plan)
                                            <div
                                                class="col-span-4 md:col-span-2 rounded-xl border border-gray-700 px-4 py-4 md:px-6 md:py-4 shadow">
                                                <h2 class="sedan-regular text-lg tracking-wide mb-2">{{ $plan->name }}
                                                </h2>
                                                <p class="text-sm mb-2">Interest rate: {{ $plan->perMonInt }}%</p>
                                                <p class="text-sm mb-2">Minimum investment amount:
                                                    ${{ number_format($plan->min) }}</p>
                                                <p class="text-sm mb-2">Maximum investment amount:
                                                    ${{ number_format($plan->max) }}</p>                                               
                                                <p class="text-sm mb-2">Duration of plan: {{ $plan->duration_str }}</p>
                                                <p class="futura-light text-sm mb-4">Created on:
                                                    {{ date('M d, y', strtotime($plan->created_at)) }}%</p>
                                                ...
                                                <div class="flex justify-end py-2 items-center">
                                                    <x-link-two href="{{ route('admin.plan.edit', [$plan->id]) }}"
                                                        class="px-4 py-1 mr-4 rounded-xl mt-2 hover:text-gray-100">Edit</x-link-two>
                                                    <form method="POST"
                                                        action="{{ route('admin.plan.destroy', [$plan->id]) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="px-4 py-2 text-gray-100 rounded-xl bg-rose-500 mt-2">
                                                            {{ __('DELETE') }}
                                                        </button>
                                                    </form>
                                                </div>
